...
- **Tool contracts** — each tool's name, parameter schema, semantics, and return shape. Tool *implementations* may be re-homed into the new module layout, but their signatures and observable behavior must not change. **This explicitly includes tools that frozen prompts name** — the `call_<agent_name>` delegation tools and the shared-knowledge tools (`clear_/get_/update_shared_knowledge`); see doc 09 for how they map onto raw ADK.
