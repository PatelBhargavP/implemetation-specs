...
**Problem B — background writes crossing turn boundaries.** A background process (execution loop, long subtask, analysis job) that writes to ADK session state after the originating turn has ended will either (a) fail/overwrite because it holds a stale session snapshot, or (b) have its update silently dropped because the turn already committed and closed. This is a *lifecycle* mismatch: background work outlives turns, but ADK session state is turn-scoped.
...
- **P3 — Separate two state domains.** *Conversational/agent state* (ADK sessions) and *execution state* (running plans, analysis jobs) are different lifecycles and live in different stores. Background workers own execution state and never touch ADK session state directly (fixes Problem B).
