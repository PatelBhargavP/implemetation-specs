...
**Problem A — OCC conflicts during a turn.** ADK's `DatabaseSessionService` uses optimistic concurrency: `append_event` checks that the session has not changed since it was loaded (stale-session detection) and raises on conflict. Any pattern where the same `session_id` is written by **more than one path within a turn** — separate `Runner` invocations, or a delegated agent persisting its result through a separate write — can race and conflict. The custom fix (sub-agents return diffs, parent aggregates, one atomic commit at end of turn) is a hand-rolled version of what ADK already provides *inside a single invocation*.

> **Root-cause note (doc 08 Q4, doc 09 §5).** Bhargav confirmed the session was **not** reloaded per sub-agent — so the "each agent reloads a different snapshot" theory is disconfirmed; agents shared one in-turn reference. The concurrency pain came from **Problem B (background writes)** and/or delegated-agent results reaching the session via a separate write, **not** from per-agent reloads. The single-invocation model below is strictly safer regardless (one writer intra-turn), and the execution decoupling (§5) is the actual fix for Problem B. Because delegation is via `call_*` tools run **in-invocation** (doc 09 §2), those delegated writes are folded into the one serial `append_event` stream — no separate writer.
...
| **Shared knowledge** (the old `SharedContext` blob; agents pass intermediate findings) | ADK session-state key `shared_knowledge` (scope `temp:` vs session per Phase 0 — doc 09 §3) | Agents via the frozen `clear_/get_/update_shared_knowledge` tools (`ToolContext` → `state_delta`) | Per doc 09 §3; class removed, data preserved |
