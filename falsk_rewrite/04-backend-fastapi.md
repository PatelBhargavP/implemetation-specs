...
- **`AUTH_MODE=local`** (valid only in the `local`/`test` profiles): `require_identity` skips header parsing and injects a **configurable dev identity** (`DEV_USER_ID` / `DEV_USER_EMAIL`, defaulting to a dev user), then proceeds through the **normal upsert + Redis-cache + `AuthContext` path**. The injected dev user is written to the `users` table and cached exactly like a real IAP user, so sessions, ownership, and authz resolve against a genuine row — downstream code cannot tell the identity is synthetic. Only the identity *source* differs from production.
- **Acting as multiple users locally (to test authz + snapshot sharing — §5.2):** a single fixed dev user can't exercise owner-only 404s or the share→new-session flow. So in the `local`/`test` profiles **only**, `require_identity` also honors a dev override to select the acting identity:
  - an `X-Dev-User` request header (or `?dev_user=` query param on the WS upgrade), matched against a small config-defined roster (e.g. `DEV_USERS: [{id,email}, …]`); if absent, fall back to `DEV_USER_ID`.
  - This override is **ignored/rejected in the `cloud` profile** (same profile gate + startup assertion as the bypass itself) — it can never be used to spoof identity in production, where identity comes solely from IAP.
  - This lets a developer open two browser profiles / send two header values and drive user A ↔ user B flows end to end.
