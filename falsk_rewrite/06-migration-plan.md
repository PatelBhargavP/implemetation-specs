...
- **Inventory current assets from `legacy/`** (doc 00 A6): extract the exact agent prompts (including any composition logic that renders them), tool signatures/return shapes, the current DB schema + Alembic history, the current REST paths (note the `/api/*` prefix — doc 04 §9), the current WS message shapes/path, the **API key manager service** (its module + public interface — doc 00 §4; capture how callers obtain a key and how rotation surfaces, so it can be wired rotation-safely per doc 03 §2.1), and the **frozen delegation & shared-knowledge tools** (the complete set of `call_<agent_name>` and `*_shared_knowledge` tools — exact names, args, returns, and the shared-knowledge state key + lifetime — doc 09). Record the `legacy/`-relative source path for each asset. Produce a checked-in `INVENTORY.md` listing every frozen asset (doc 00 §4) with its source location and a content hash. **Gate:** inventory reviewed and signed off before any agent code is copied.
- **Read the legacy docs & READMEs too.** As part of the inventory, read every `README`, doc, design note, and inline docstring in `legacy/` and mine them for **domain/business knowledge** — assay logic, deck/resource-estimation rules, the intended meaning of agents and the `call_*` / shared-knowledge tools (doc 09), config/runbook details, and known gotchas. Capture anything that clarifies *what the system computes or intends* in `INVENTORY.md` (or a short `DOMAIN-NOTES.md`), with the `legacy/` source path. These docs frequently explain behavior that code alone doesn't.
  - **Treat nearly every `legacy/**/*.md` as instructional documentation.** Markdown across the legacy tree is documentation, not decoration — read all of it as domain/intent context (subject to the precedence rule: docs inform, code wins).
  - **Known location: `legacy/features/*`.** Much of it is organized **per feature** under `features/*` (each subfolder documents one capability — e.g. planning, deck/resource estimation, execution, analysis, sharing). Walk this tree in full; treat each feature doc as the primary narrative for that capability's intended behavior, and cross-check it against the corresponding frozen code. Do **not** assume the top-level `README` is the whole picture.
  - **EXCEPTION — agent/skill markdown are FROZEN ASSETS, not advisory context.** Markdown that *defines an agent or a skill* (agent instructions/prompts, skill docs/definitions) is very likely the live prompt/skill content itself. Treat these as **frozen** (doc 00 §4, doc 03 §1, doc 09): preserve byte-identical, hash them, and port their rendered content verbatim — do **not** paraphrase, "improve", or treat them as mere reference. When in doubt whether an `.md` is documentation or a frozen agent/skill definition, check whether the code loads it at runtime as an instruction/prompt; if so, it is frozen.
- **Precedence rule (resolve conflicts in this order):**
  1. **These specs (`docs/specs/00`–`09`) govern the harness/architecture** — server, state/concurrency, wiring, deployment. Legacy docs describe the *old* architecture (Quart, Turn-Aware, monolith) that is being replaced; **do not treat their design as binding** and do not port them forward.
  2. **Legacy code is authoritative for frozen-asset contracts** (prompt text, tool names/signatures/returns, schema).
  3. **Legacy docs/READMEs are context, not authority** — they inform domain intent, but where a doc conflicts with the code, **code wins** (docs may be stale or AI-generated slop). Where a legacy doc conflicts with *these specs on architecture*, **these specs win**. Flag any material conflict in `docs/specs/08-open-questions.md` rather than guessing.
